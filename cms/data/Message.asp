...
Select Case action
	Case "read"
		...
		Set rs = conn.Execute("select count(*) from Message")
		...
		If page > 0 Then
			...
			sql = "SELECT * FROM [select top " & pagesize & " a.* from (select top "& limit * page & " * from Message where category = " & category & " ORDER BY SortIndex DESC) a left join (select top " & limit * page & " ID, SortIndex from Message where category = " & category & " ORDER BY SortIndex DESC) b on b.ID=a.ID order by a.sortindex asc]. AS c ORDER BY sortindex DESC;"

		Else
			sql = "select TOP "& limit &" * from Message order by sortindex DESC"
		End If
	...
	Case "create"
		...
		comm.commandtext = "INSERT INTO Message(Title, Fullname, Email, Telephone, Reply, Category, [Public]) VALUES(:Title, :Fullname, :Email, :Telephone, :Reply, :Category, :Public)"

		comm.Parameters.Append comm.CreateParameter(":Title", adVarChar, adParamInput, 255, autoaModel.Title)
		comm.Parameters.Append comm.CreateParameter(":Fullname", adVarChar, adParamInput, 255, autoaModel.Fullname)
		comm.Parameters.Append comm.CreateParameter(":Email", adVarChar, adParamInput, 50, autoaModel.Email)
		comm.Parameters.Append comm.CreateParameter(":Telephone", adVarChar, adParamInput, 50, autoaModel.Telephone)
		comm.Parameters.Append comm.CreateParameter(":Reply", adVarChar, adParamInput, 50, autoaModel.Reply)
		comm.Parameters.Append comm.CreateParameter(":Category", adBigInt, adParamInput, 32, category)
		comm.Parameters.Append comm.CreateParameter(":Public", adBoolean, adParamInput, 255, autoaModel.Public)
...
'		rst.CursorLocation = 3
		...
		sqlStr = "UPDATE Message SET SortIndex = '" & LastOrderId & "' WHERE ID = " & LastOrderId
		conn.execute(sqlStr)
	...
	Case "update"
		...
		comm.ActiveConnection = conn 
		comm.commandtype = 1 
		comm.commandtext = "UPDATE Message Set Title = :Title, Fullname = :Fullname, Reply = :Reply, Email = :Email, Telephone = :Telephone, MODDatetime = Now(), SortIndex = :SortIndex, [Public] = :Public WHERE ID = :ID"

		comm.Parameters.Append comm.CreateParameter(":Title", adVarChar, adParamInput, 255, autoaModel.Title)
		comm.Parameters.Append comm.CreateParameter(":Fullname", adVarChar, adParamInput, 255, autoaModel.Fullname)
		comm.Parameters.Append comm.CreateParameter(":Reply", adVarChar, adParamInput, 255, autoaModel.Reply)
		comm.Parameters.Append comm.CreateParameter(":Email", adVarChar, adParamInput, 50, autoaModel.Email)
		comm.Parameters.Append comm.CreateParameter(":Telephone", adVarChar, adParamInput, 50, autoaModel.Telephone)
...
'ID', 'Title', 'Reply', 'Email', 'Telephone', 'ADDDatetime', 'MODDatetime', 'SortIndex', 'Category'
...
'Set modeling("datalist") = recorder
	Case "destroy"
		...
		If Err.Number <> 0 Then
			'单条记录
			comm.commandtext = "delete from Message where ID = :ID"
			comm.Parameters.Append comm.CreateParameter(":ID", adVarChar, adParamInput, 50, autoaModel.ID)
		Else
			'多条记录
			iArrayTempString = "delete from Message where ID IN("
