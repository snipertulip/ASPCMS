...
Select Case action
	Case "read"
		...
		Set rs = conn.Execute("select count(*) from ArticleList where category = " & category & "")
		...
		If page > 0 Then
			...
			sql = "SELECT * FROM [select top " & pagesize & " a.* from (select top "& limit * page & " * from ArticleList where category = " & category & " ORDER BY SortIndex DESC) a left join (select top " & limit * page & " ID, SortIndex from ArticleList where category = " & category & " ORDER BY SortIndex DESC) b on b.ID=a.ID order by a.sortindex asc]. AS c ORDER BY sortindex DESC;"

		Else
'			sql = "select TOP "& limit &" * from ArticleList order by sortindex DESC"
		End If
	...
	Case "create"
		...
		comm.commandtext = "INSERT INTO ArticleList(Title, Summary, Content, Attachment, Source, Author, Label, ExternalLinks, Category) VALUES(:Title, :Summary, :Content, :Attachment, :Source, :Author, :Label, :ExternalLinks, :Category)"
		...
		comm.Parameters.Append comm.CreateParameter(":Author", adVarChar, adParamInput, 255, autoaModel.Author)
...
'		rst.CursorLocation = 3
		...
		sqlStr = "UPDATE ArticleList SET SortIndex = '" & LastOrderId & "' WHERE ID = " & LastOrderId
		conn.execute(sqlStr)
	...
	Case "update"
		...
		comm.ActiveConnection = conn 
		comm.commandtype = 1 
		comm.commandtext = "UPDATE ArticleList Set Title = :Title, Summary = :Summary, Content = :Content, Attachment = :Attachment, Source = :Source, Author = :Author, Label = :Label, ExternalLinks = :ExternalLinks, MODDatetime = Now(), SortIndex = :SortIndex WHERE ID = :ID"
...
'Set modeling("datalist") = recorder
	Case "destroy"
		...
		If Err.Number <> 0 Then
			'单条记录
			comm.commandtext = "delete from ArticleList where ID = :ID"
			comm.Parameters.Append comm.CreateParameter(":ID", adVarChar, adParamInput, 50, autoaModel.ID)
		Else
			'多条记录
			iArrayTempString = "delete from ArticleList where ID IN("
