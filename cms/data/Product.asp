...
Dim toCategory : toCategory = C2Lng(Request("toCategory"))
...
Select Case action
	Case "read"
		...
		Set rs = conn.Execute("select count(*) from Product where category=" & category)
		...
		If page > 0 Then
			...
			sql = "SELECT * FROM [select top " & pagesize & " a.* from (select top "& limit * page & " * from Product where category = " & category & " ORDER BY SortIndex DESC) a left join (select top " & limit * page & " ID, SortIndex from Product where category = " & category & " ORDER BY SortIndex DESC) b on b.ID=a.ID order by a.sortindex asc]. AS c ORDER BY sortindex DESC;"

		Else
'			sql = "select TOP "& limit &" * from Product order by sortindex DESC"
		End If
	...
	Case "create"
		...
		comm.commandtext = "INSERT INTO Product(Title, Summary, Content, Attachment, Source, Attachment1, Label, ExternalLinks, Category, Recommend) VALUES(:Title, :Summary, :Content, :Attachment, :Source, :Attachment1, :Label, :ExternalLinks, :Category, :Recommend)"
		...
		comm.Parameters.Append comm.CreateParameter(":Attachment", adVarChar, adParamInput, 255, autoaModel.Attachment)
		comm.Parameters.Append comm.CreateParameter(":Source", adVarChar, adParamInput, 255, autoaModel.Source)
		comm.Parameters.Append comm.CreateParameter(":Attachment1", adVarChar, adParamInput, 255, autoaModel.Attachment1)
		comm.Parameters.Append comm.CreateParameter(":Label", adVarChar, adParamInput, 255, autoaModel.Label)
		comm.Parameters.Append comm.CreateParameter(":ExternalLinks", adVarChar, adParamInput, 255, autoaModel.ExternalLinks)
		comm.Parameters.Append comm.CreateParameter(":Category", adBigInt, adParamInput, 32, category)
		comm.Parameters.Append comm.CreateParameter(":Recommend", adBoolean, adParamInput, 255, autoaModel.Recommend)
...
'		rst.CursorLocation = 3
		...
		sqlStr = "UPDATE Product SET SortIndex = '" & LastOrderId & "' WHERE ID = " & LastOrderId
		conn.execute(sqlStr)
	...
	Case "update"
		...
		comm.ActiveConnection = conn 
		comm.commandtype = 1 
		comm.commandtext = "UPDATE Product Set Title = :Title, Summary = :Summary, Content = :Content, Attachment = :Attachment, Source = :Source, Attachment1 = :Attachment1, Label = :Label, ExternalLinks = :ExternalLinks, MODDatetime = Now(), SortIndex = :SortIndex, Recommend = :Recommend WHERE ID = :ID"
...
'Set modeling("datalist") = recorder
	Case "destroy"
		...
		If Err.Number <> 0 Then
			'单条记录
			comm.commandtext = "delete from Product where ID = :ID"
			comm.Parameters.Append comm.CreateParameter(":ID", adVarChar, adParamInput, 50, autoaModel.ID)
		Else
			'多条记录
			iArrayTempString = "delete from Product where ID IN("
	...
	Case "move"
		...
		Err.Clear
		iArrayTempString = VBNullString 
		iArrayLength = autoaModel.length
		iArrayLength = iArrayLength XOR iArrayLength

		If Err.Number <> 0 Then
			'单条记录
			comm.commandtext = "update Product set Category = :toCategory where ID = :ID"
			comm.Parameters.Append comm.CreateParameter(":toCategory", adVarChar, adParamInput, 50, toCategory)
			comm.Parameters.Append comm.CreateParameter(":ID", adVarChar, adParamInput, 50, autoaModel.ID)
		Else
			'多条记录
			iArrayTempString = "update Product set Category = :toCategory where ID IN("
			Do While iArrayLength < autoaModel.length - 1
				iArrayTempString = iArrayTempString & ":ID" & iArrayLength & ", "
				iArrayLength = iArrayLength + 1
			Loop
	...
	Case "moveall"
		...
		If Err.Number <> 0 Then
			'单条记录
			comm.commandtext = "update Product set Category = :toCategory where Category = :Category"
			comm.Parameters.Append comm.CreateParameter(":toCategory", adVarChar, adParamInput, 50, toCategory)
			comm.Parameters.Append comm.CreateParameter(":Category", adVarChar, adParamInput, 50, Category)
		Else
'			'多条记录
'			iArrayTempString = "update Product set Category = :toCategory where ID IN("
'			Do While iArrayLength < autoaModel.length - 1
