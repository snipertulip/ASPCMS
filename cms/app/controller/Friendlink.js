Ext.define('CMS.controller.Friendlink', {
	extend: 'Ext.app.Controller',

	requires: ['Ext.ux.UrlParameters'],

	views: ['friendlink.List', 'friendlink.Create'
			,'friendlink.Edit'
	],
	...
	_DestroyData: function(button) {
		...
		store.sync({
			success: function(batch, option) {
				...
				//				var friendlinkList = Ext.widget('gridlist');
				//				friendlinkList.getStore().load();
	...
	_CreateData: function(button) {
		...
		//			values = form.getValues();
		//		record.set(values);
		var record = new Ext.create(this.getModel('Friendlink'));
		record.set(form.getValues());

		win.close();
		this.getFriendlinkStore().insert(0, record);
		this.getFriendlinkStore().sync({
				success: function(batch, option) {
					//alert(this.getReader().rawData.msg);
					//var friendlinkList = Ext.widget('gridlist');
	...
	_SelectionChange: function(view, records) {
		Ext.getCmp('gridlist').down('#removeFriendlink').setDisabled(!records.length);
		Ext.getCmp('gridlist').down('#editFriendlink').setDisabled(records.length === 1 ? false : true);
	},
