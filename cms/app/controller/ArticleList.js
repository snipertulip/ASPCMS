Ext.define('CMS.controller.ArticleList', {
	extend: 'Ext.app.Controller',

	requires: ['Ext.ux.UrlParameters'],

	views: ['articlelist.List', 'articlelist.Create'
			,'articlelist.Edit'
	],
	...
	init: function() {
		...
		//		this.getStore('ArticleList').load();
	...
	_DestroyData: function(button) {
		...
		store.sync({
			success: function(batch, option) {
				...
				//				var articlelistList = Ext.widget('gridlist');
				//				articlelistList.getStore().load();
	...
	_CreateData: function(button) {
		...
		//			values = form.getValues();
		//		record.set(values);
		var record = new Ext.create(this.getModel('ArticleList'));
		record.set(form.getValues());

		win.close();
		this.getArticleListStore().insert(0, record);
		this.getArticleListStore().sync({
				success: function(batch, option) {
					//alert(this.getReader().rawData.msg);
					//var articlelistList = Ext.widget('gridlist');
	...
	_SelectionChange: function(view, records) {
		Ext.getCmp('gridlist').down('#removeArticleList').setDisabled(!records.length);
		Ext.getCmp('gridlist').down('#editArticleList').setDisabled(records.length === 1 ? false : true);
	},
