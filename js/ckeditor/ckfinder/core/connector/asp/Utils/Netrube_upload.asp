...
Class NetRube_Upload
	...
	Public	File, Form
	Private oSourceData
	Private nMaxSize, nErr, sErrDescription, sAllowed, sDenied, sHtmlExtensions
	...
	Public Sub GetData
		...
		' Detection of script time out:
		limitTime = DateAdd("s", Server.ScriptTimeOut, Now())
		...
		Do While ReadBytes < nTotalBytes
			...
			If (Err.number<>0) Then
				...
				nErr = 8 ' error while processing the upload, probably the AspMaxRequestEntityAllowed value
				Exit Sub
			End If
			If (Now()>limitTime) then
				sErrDescription = "Script Time out"
				nErr = 9
				Exit Sub
			End If
		Loop
		...
		Dim oTotalData, oFormStream, sFormHeader, sFormName, bCrLf, nBoundLen, nFormStart, nFormEnd, nPosStart, nPosEnd, sBoundary, nBoundaryStart
		Dim currentFile
		...
		nBoundaryStart = InStrB(1, oTotalData, bCrLf)
		If (Err.number<>0) Then
			sErrDescription = Err.Description
			nErr = 8 ' error while copying data, probably not enough memory
			Exit Sub
		End If
		If (nBoundaryStart=0) Then
			nErr = 7 ' Incorrect format of the data
			Exit Sub
		End If

		sBoundary	= MidB(oTotalData, 1, nBoundaryStart - 1)
		...
		Do While (nFormStart + 2) < nTotalSize
			nFormEnd = InStrB(nFormStart, oTotalData, bCrLf & bCrLf)
			If (nFormEnd<1) Then
				nErr = 7 ' Incorrect format of the data
				Exit Sub
			End If
			...
			If (nPosEnd<1 Or nFormStart<1) Then
				nErr = 7 ' Incorrect format of the data
				Exit Sub
			End If
			...
			If InStr(45, sFormHeader, " filename=", 1) > 0 Then
				Set currentFile			= New NetRube_FileInfo
				currentFile.FormName	= sFormName
				currentFile.Start		= nFormEnd
				currentFile.Size		= nFormStart - nFormEnd - 2
				nPosStart					= InStr(nPosEnd, sFormHeader, " filename=", 1) + 11
				nPosEnd						= InStr(nPosStart, sFormHeader, """")
				currentFile.ClientPath	= Mid(sFormHeader, nPosStart, nPosEnd - nPosStart)
				currentFile.Name		= Mid(currentFile.ClientPath, InStrRev(currentFile.ClientPath, "\") + 1)
				currentFile.Ext			= LCase(Mid(currentFile.Name, InStrRev(currentFile.Name, ".") + 1))
				nPosStart					= InStr(nPosEnd, sFormHeader, "Content-Type: ", 1) + 14
				nPosEnd						= InStr(nPosStart, sFormHeader, vbCr)
				currentFile.MIME		= Mid(sFormHeader, nPosStart, nPosEnd - nPosStart)
				File.Add sFormName, currentFile
			Else
				With oFormStream
					.Type	= 1
					.Mode	= 3
	...
	Public Sub SaveAs(sItem, sFileName)
		Dim currentFile
		Set currentFile = File(sItem)
		If currentFile.Size < 1 Then
			nErr = 2
			Exit Sub
		End If

		If Not IsAllowed(currentFile.Ext) Then
			nErr = 4
			Exit Sub
		End If
		...
		sFileExt	= currentFile.Ext
		iFileSize	= currentFile.Size
		...
		If Not IsHtmlExtension( sFileExt ) Then
			...
			' Read the data.
			Dim sData
			oSourceData.Position = currentFile.Start
		...
		With oFileStream
			...
			.Open
			oSourceData.Position = currentFile.Start
			oSourceData.CopyTo oFileStream, currentFile.Size
			.Position	= 0
